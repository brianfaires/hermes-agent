...
class Run:
    ...
    def preflight(self):
        ...
        require(self.p['local']['rollback'] == self.p['rollback']['sha'], 'rollback not preserved')
        require(self.p['local']['staging'] == self.p['candidate']['sha'] == self.p['authoritative']['staging'], 'candidate not qualified staging')
        require(self.p['local']['main'] == self.p['authoritative']['main'], 'main alignment')
        ...
        if self.p['operation'] == 'promote':
            git(self.p['repo'], 'merge-base', '--is-ancestor', self.p['local']['main'], self.p['candidate']['sha'])
        self.clock_gate()
        ...
        with self.lock():
            ...
            for cmd in self.p['commands']:
                ...
                if cmd['id'] == 'switch':
                    self.refs()
                if cmd['id'] == 'merge':
                    require(git(self.p['repo'], 'rev-parse', 'HEAD') == self.p['local']['main'], 'main parent drift')
                if cmd['id'] == 'push':
    ...
    def recovery_refs(self):
        ...
        self.extra_refs()
        require(git(self.p['repo'], 'rev-parse', 'main') in {self.p['local']['main'], self.p['candidate']['sha']}, 'recovery main drift')
        require(git(self.p['repo'], 'rev-parse', 'staging') == self.p['candidate']['sha'], 'recovery staging drift')

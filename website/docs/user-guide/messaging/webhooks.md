...
| `oidc.email` | No | Expected verified service-account email claim. When configured, restricts which Pub/Sub push service account may call the route. |
| `oidc.subject` | No | Expected JWT `sub` claim. Use the immutable service-account subject when available. If both identity fields are configured, both must match. |
...
Hermes verifies the token signature against Google's JWKS, checks the audience, and accepts Google's standard issuers. To restrict the route to one caller, configure `oidc.email` or `oidc.subject`; identity matching is optional globally but recommended for internet-facing routes. If your Pub/Sub subscription uses a custom OIDC audience, put that exact value in `oidc.audience`.

...
| Command | Description |
|---------|-------------|
| `/new [name]` or `/new (<prompt>)` (alias: `/reset`) | Start a new session (fresh session ID + history). Optional `[name]` sets the initial session title — e.g. `/new my-experiment` opens a fresh session already titled `my-experiment` so it's easy to find later with `/resume` or `/sessions`. Use `/new (<prompt>)` to start a fresh session and send `<prompt>` as its first message. Append `now`, `--yes`, or `-y` to skip the confirmation modal — e.g. `/reset now`, `/new --yes my-experiment`. |
...
| `/new [name]` or `/new (<prompt>)` (alias: `/reset`) | Start a new session (fresh session ID + history). Optional `[name]` sets the initial session title. Use `/new (<prompt>)` to start a fresh session and send `<prompt>` as its first message. Append `now`, `--yes`, or `-y` to skip the confirmation modal — e.g. `/reset now`, `/new --yes my-experiment`. |

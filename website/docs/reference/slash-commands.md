...
| `/new [name]` or `/new (<prompt>)` (alias: `/reset`) | Start a new session (fresh session ID + history). Optional `[name]` sets the initial session title. `/new (<prompt>)` (messaging gateway only) resets **and** sends `<prompt>` as the new session's first message — e.g. `/new (summarize yesterday's deploy)`; the reset banner arrives first and the agent's answer follows as the reply. The parenthesized form never sets a title, and a prompt starting with `/` is sent literally, not executed as another command. Duplicate transport events are suppressed within the existing process-local 300-second/2,000-entry cache when a message ID is available; restart or eviction ends that protection. Empty parentheses (`/new ()`, `/new (`) carry no message and behave exactly like a bare `/new`; a missing closing paren still sends everything after `(` as the prompt rather than truncating it into a title. Append `now`, `--yes`, or `-y` to skip the confirmation modal — e.g. `/reset now`, `/new --yes my-experiment`. |

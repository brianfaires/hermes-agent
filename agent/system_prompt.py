...
from agent.prompt_builder import (
    ...
    resolve_model_execution_guidance,
)
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # process for one task (HERMES_KANBAN_TASK env var). Orchestrator profiles
    # may expose kanban_show via the explicit kanban toolset, so tool presence
    # is not a worker-session signal. Resolved once at __init__ (see
    ...
    if agent.valid_tool_names:
        ...
        if _inject:
            ...
            _execution_guidance = resolve_model_execution_guidance(_model_lower)
            if _execution_guidance:
                stable_parts.append(_execution_guidance)

...
def on_complete(job: dict, success: bool, duration_seconds: float,
    ...
    # Resize Calendar for every new maximum, but only alert Brian when the
    # learned bound materially moves. A +1s record is bookkeeping, not news.
    alert = duration > (prior_val * 1.5)
    if not alert:
        logger.info(
            "[%s] increased cron max_duration from %.0fs -> %.0fs below alert threshold",

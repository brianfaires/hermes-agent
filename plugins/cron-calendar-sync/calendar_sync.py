...
def _upsert(job: dict) -> None:
    ...
    for extra in [*tracked, *adoptable]:
        ...
        if not extra_id or extra_id == event_id or extra_id in seen_extra_ids:
            continue
        seen_extra_ids.add(extra_id)
        if not _archive_event(calendar, extra_id, "superseded duplicate managed series"):
            private = ((extra.get("extendedProperties") or {}).get("private") or {})
            retained_events.append(
                {
                    "event_id": extra_id,
                    "start": extra.get("start"),
                    "mode": extra.get("mode") or private.get("hermesCronCalendarMode"),
                }
            )

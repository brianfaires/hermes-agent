...
def _wildcard_cleanup_plan(item: Dict[str, Any], now: datetime) -> Tuple[List[Dict[str, Any]], List[Path]]:
    ...
    for child in _iter_descendants(parent):
        ...
        try:
            ...
            # Files under mutable cache/output trees may disappear between
            # discovery and metadata collection. Treat that as already clean.
            continue
        if not stat.S_ISREG(child_stat.st_mode):
            continue
        age = (
            now - datetime.fromtimestamp(child_stat.st_mtime, tz=timezone.utc)
        ).days

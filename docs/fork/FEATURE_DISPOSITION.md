...
RETAINED-FEATURE INTEGRATION UPDATE (2026-09-07). Brian authorized all five reviewed candidates; they are integrated in release-owner task `t_ddd2e9dc` on doctor baseline `9ccb53e3d15730fcae88b086ea954dfc377574aa`. FC-11/37/41/42/44 are implemented candidates; FC-43 is explicitly SUPERSEDED as described below. Source implemented does not mean deployed, CI-passed or activation-passed; this is source integration, not whole-rebase completion, hosted-CI qualification, deployment or feature activation. Earlier FC-16/22/28B/36 approvals and all other feature decisions remain unchanged. Current durable receipts are the parent `t_98a41867` `INTEGRATION.txt`, root `t_ddd2e9dc` `release-checkpoint.json` and `final-source-review.md`, and qualification task `t_5bb4934d`'s `QUALIFICATION.txt` and `qualification-checks.json`. Historical integration evidence remains in the parent task’s `integration-evidence.json` and `integration-result.json`.
...
- Allowed decisions are `KEEP`, `REWRITE`, `DROP_UPSTREAM`, `DROP_LOW_VALUE`, `DROP_OUT_OF_SCOPE`, `DEFER_HUMAN_VALUE`, `DEFER_REPRODUCTION`, and `SUPERSEDED`.
...
| FC-05 | Hindsight availability is independent of the file-memory toolset | I005 | `KEEP` | Live gap confirmed on recon tip: provider tools still gated solely on the file-backed `memory` toolset, so `disabled_toolsets: [memory]` withheld Hindsight retain/recall/reflect. Added empty `hindsight` toolset, auto-enable of `memory.provider` toolset name, and provider-named gate in `memory_provider_tools_exposed`; focused gate + tools_config tests pass. | Existing memory-provider injection path + tools config only. |
| FC-06 | Tool-search platform pinning keeps required tools visible | I006 | `DROP_LOW_VALUE` | Peer audit: `pinned_toolsets` is absent on recon/clean (not already upstream). Read-only live config has the key empty with no current consumer; core tools remain never-deferred. Empty-consumer drop, not DROP_UPSTREAM. | Recover only with a real platform consumer and fresh placement review. |
...
| FC-11 | Google Calendar cron synchronization and lifecycle reconciliation | I011, I014, I046, I057, I058, I110, I124, I133 | `REWRITE` | Implemented candidate from `9d43838c6ca07e6d9cb91786266aec23fc9233ec`: durable cron execution ledger and private Calendar mirror. Retains final responses rather than transcripts, including removed one-shots; owner cancellation discards the cancelled answer. Existing reviewed source accepted by Ang; no live Calendar calls in this integration. | Ops prerequisite `t_907703d2`: sole-owner Calendar, grants/config and real-result UAT remain required. No enablement or production compatibility claim; old hook/event-bus framework is not restored. |
...
| FC-37 | Hindsight history reconstruction command | I054 | `KEEP` | Implemented OS-owner CLI from the two-commit range `9ccb53e3d15730fcae88b086ea954dfc377574aa..3b93f82d45e5bdc0830d560561c6503d665d23b4`, squashed once with final head content in all three new paths. Default recorded-evidence export; explicitly selected current-memory reconstruction is separately labeled. Scoped ownership/session authorization and provenance fail closed, including WAL evidence. | Standalone `scripts/hindsight_history/cli.py`, NOT a chat slash command. Reconstruction supports cloud/local_external with either a valid injective `{profile}` bank template or an explicit static `bank_id` / legacy `banks.hermes.bankId` when the template is absent or empty. Malformed templates fail closed. No claim of live compatibility, live queries, history import, retention change or stored-data migration. |
...
| FC-41 | Gateway `/new (<prompt>)` shorthand | I139 | `REWRITE` | Implemented `/new` candidate from `10813a6f2e230cf016a8a2ebd24125972edf10a3`, carried once before its `/log` child. Reset delivers the parenthesized prompt as literal user text in the source profile. Bare/title behavior, confirmation/cancellation and busy interruption remain. Duplicate suppression uses the existing process-local 300-second/2,000-entry transport-ID cache, not crash-durable exactly-once delivery. | Gateway surface; source-profile confirmation and history isolation preserved. Parent’s 111 passing checks and closed review accepted. Integrated but not deployed; exact-SHA hosted staging CI remains a release gate. |
| FC-42 | Private `/log` journal capture and nightly batch | I141, I153 | `REWRITE` | Implemented private `/log` candidate from `de66979c428427d28457763682406e685bd9a220`, the linear child of FC-41. Early private ingress precedes session/model/history handling. One isolated midnight batch, default raw retention, durable finalized archive/receipts and explicit uncertainty reconciliation; intentional finalized-entry-only Hindsight ingestion into the correct profile scope. | CLI/TUI plus authorized Telegram/Discord delivered text only; no voice/media/desktop/split continuations. Disabled profile plugin, installation examples under `plugins/private_journal/installation/`. Parent’s 385 Python + 90 TUI checks accepted. Ang release owner `t_ddd2e9dc`; Ops activation prerequisite `t_907703d2`. Destructive cleanup off; interval undecided. No accidental chat ingestion or mass backfill/deletion. |
| FC-43 | Block Personal History Log retains in Hindsight | I142 | `SUPERSEDED` | Brian explicitly superseded the historical blanket Personal History Log journal-to-Hindsight block with intentional finalized journal ingestion in the correct profile scope. The individual historical item remains in the immutable inventory once; its blanket block is not replayed. | FC-42 owns the intentional finalized-entry boundary. No accidental chat ingestion, mass backfill/deletion or new global name-based memory policy. |
| FC-44 | Inactive release-switch controller | I143, I144, I145, I146, I147, I148, I149, I150 | `REWRITE` | New standalone sandbox-qualified controller from `b9c93e2380210049699e03073fe707db8495ff99`, with deterministic frozen-packet execution and fenced recovery. Original framework is not restored. Parent’s 89 disposable + 16 portable checks are accepted; unchanged deliverable bytes do not require another disposable-service run. | `scripts/claude_release_switch/`; NOT installed or production-qualified. Lifecycle/topology, frozen authority, readiness/health, rollback and recovery prerequisites must be proven by parent Ang separately. No Claude call, service operation or controller execution against production in this integration. |
...
Historical reference only: these ten boundaries and the old incident/reconstruction graphs remain held, including `t_6d9a0f05`, `t_99b38c75` and their held descendants. Do not release, execute or auto-decompose them. Current release owner is `t_ddd2e9dc`; Ops prerequisite is `t_907703d2`. New accepted feature decisions do not release old cards.
...
- Historical milestone counted 51 cluster rows. Its old disposition breakdown is superseded by the programmatically computed current counts above; inventory identities remain unchanged.
...
## Historical main/staging reconciliation (2026-09-06 snapshot)

At that snapshot, local main `2945588a014543d47c9e5e4a0d92ba6e361387c1` exactly equals
...
At that snapshot, staging `9ff6594cc9227ef15fe0d586349e2793ab10c6a3` adds exactly two
...
inventory. Nothing was merged wholesale. The current retained-feature decisions above supersede the earlier FC-11/37/41–44 omissions. Historical tables naming held follow-up cards remain lineage only, not instructions to dispatch them.

...
def write_fake_hermes(path: Path, log_path: Path, *, fail_start: bool = False) -> None:
    exit_expr = "exit 3" if fail_start else "exit 0"
    path.write_text(
        "#!/usr/bin/env bash\n"
        "set -eu\n"
        f"printf '%s\\n' \"$*\" >> {str(log_path)!r}\n"
...
def write_receipt(path: Path, *, sha: str, now: str = "2026-08-20T12:00:00Z", ok: bool = True) -> Path:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        json.dumps({"sha": sha, "status": "ok" if ok else "failed", "issued_at": now}),
        encoding="utf-8",
    )
    return path
...
def write_config(
    ...
    *,
    ...
    write_fake_hermes(fake_hermes, lifecycle_log, fail_start=start_fails)
    smoke = tmp_path / "smoke.json"
    smoke.write_text(json.dumps({"ok": smoke_ok, "pid": 2222, "source": str(checkout), "sha": sha}), encoding="utf-8")
    ...
    runtime.write_text(json.dumps({"ok": True, "pid": 1111, "source": str(checkout)}), encoding="utf-8")
    auth_path = receipts / "authorization.json"
    if authorization:
        write_receipt(auth_path, sha=sha)
    config = {
        ...
        "compatibility_receipt": str(write_receipt(receipts / "compat.json", sha=sha)),
        "backup_receipt": str(write_receipt(receipts / "backup.json", sha=backup_sha or sha)),
        ...
        "probes": {
            ...
            "smoke": [sys.executable, "-c", f"import pathlib; print(pathlib.Path({str(smoke)!r}).read_text())"],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda checkout, sha, tmp_path: (write_receipt(tmp_path / "receipts" / "backup.json", sha=sha, now="2026-08-19T10:00:00Z"), None)[1], "stale_receipt"),

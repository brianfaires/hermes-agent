...
class TestStaleInflightSelfHeal:
    ...
    def test_stale_claim_self_heals_and_redispatches(self, cron_env, monkeypatch):
        ...
        S._running_job_ids.add(S._running_job_key(job_id))
        S._running_since[S._running_job_key(job_id)] = time.time() - 6 * 60 * 60
    ...
    def test_guard_stats_reported(self, cron_env, monkeypatch):
        ...
        S._running_since.clear()
        S._running_futures.clear()
        S._running_job_ids.add(S._running_job_key(env["job_id"]))
        S._running_since[S._running_job_key(env["job_id"])] = time.time() - 6 * 60 * 60

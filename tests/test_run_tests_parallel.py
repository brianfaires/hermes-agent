...
def test_pytest_subprocess_constrains_nested_native_thread_pools(tmp_path, monkeypatch):
    captured_env = {}
    ...
    for name in (
        ...
        assert captured_env[name] == "1"
    assert captured_env["UNRELATED_SENTINEL"] == "preserved"

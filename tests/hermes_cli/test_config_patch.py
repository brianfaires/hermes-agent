...
def test_unset_and_patch_do_not_lose_concurrent_updates(hermes_home):
    """Every config.yaml read-modify-write path must share the same lock."""
    import multiprocessing
    import time

    if "fork" not in multiprocessing.get_all_start_methods():
        pytest.skip("deterministic interleaving test requires fork")

    config_path = hermes_home / "config.yaml"
    config_path.write_text("remove_me: true\nkeep: 1\n")
    ctx = multiprocessing.get_context("fork")
    unset_has_read = ctx.Event()
    release_unset = ctx.Event()
    patch_done = ctx.Event()

    def unset_worker():
        os.environ["HERMES_HOME"] = str(hermes_home)
        original_load = config.fast_safe_load

        def paused_load(stream):
            loaded = original_load(stream)
            unset_has_read.set()
            assert release_unset.wait(10)
            return loaded

        config.fast_safe_load = paused_load
        config.unset_config_value("remove_me")

    def patch_worker():
        os.environ["HERMES_HOME"] = str(hermes_home)
        config.apply_config_patch("add", "/independent", "2")
        patch_done.set()

    unset_process = ctx.Process(target=unset_worker)
    patch_process = ctx.Process(target=patch_worker)
    unset_process.start()
    assert unset_has_read.wait(10)
    patch_process.start()
    time.sleep(0.25)
    release_unset.set()
    unset_process.join(10)
    patch_process.join(10)

    assert unset_process.exitcode == 0
    assert patch_process.exitcode == 0
    assert patch_done.is_set()
    assert read_config(hermes_home) == {"keep": 1, "independent": 2}

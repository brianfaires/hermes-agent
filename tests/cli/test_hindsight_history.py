...
def installed_sdk_env():
    return dict(os.environ, PYTHONDONTWRITEBYTECODE="1", PYTHONPATH=str(ROOT))


def installed_sdk_available(python, env):
    probe = subprocess.run([python, "-c", "import hindsight_client"], env=env,
                           capture_output=True, text=True)
    if probe.returncode:
        pytest.skip("optional Hindsight SDK is not installed in the selected test interpreter")


def run_installed_sdk_case(profile, tmp_path, *, config_overrides, response_results=None,
                           response_status=200):
    python = installed_sdk_python()
    env = installed_sdk_env()
    installed_sdk_available(python, env)
    requests = []
    ...
    class Transport(BaseHTTPRequestHandler):
        def do_POST(self):
            ...
            payload = {"results": [normalize(item) for item in (
                response_results if response_results is not None else
                [fact(), fact(id="fact-2", type="experience")]
            )]}
    ...
    try:
        ...
        config["api_url"] = f"http://127.0.0.1:{server.server_port}"
...
inspect.signature(Hindsight.recall).bind(None, bank_id="{bank_id}", query="synthetic",
...
'''.format(bank_id=config_overrides.get("bank_id") or
           config_overrides.get("banks", {}).get("hermes", {}).get("bankId", "hermes"))
...
@pytest.mark.parametrize("template", ["hermes", "hermes-{user}", "hermes-{session}",
...
@pytest.mark.parametrize("config_overrides,expected_bank_id", [
    ({"mode": "local_external", "api_url": "http://synthetic.invalid",
      "bank_id": "hermes-alpha", "bank_id_template": ""}, "hermes-alpha"),
    ({"mode": "local_external", "api_url": "http://synthetic.invalid",
      "bank_id_template": "", "banks": {"hermes": {"bankId": "hermes-alpha"}}}, "hermes-alpha"),
])
def test_installed_sdk_static_bank_positive(profile, tmp_path, config_overrides, expected_bank_id):
    proc, requests, output = run_installed_sdk_case(profile, tmp_path,
    ...
    method, path, body, auth = requests[0]
    assert auth is None
    assert (method, path) == ("POST", f"/v1/default/banks/{expected_bank_id}/memories/recall")
    assert body["tags"] == ["session:requested"] and body["tags_match"] == "all_strict"
    report = json.loads(output.read_text())
    assert report["reconstruction"]["status"] == "reconstructed"
    assert report["reconstruction"]["facts"][0]["text"] == "synthetic recovered fact"


def test_installed_sdk_mixed_wrong_profile_response_fails_closed(profile, tmp_path):
    config = {"mode": "local_external", "api_url": "http://synthetic.invalid",
              "bank_id": "hermes-alpha", "bank_id_template": ""}
    mixed = [
        fact(),
        fact(id="fact-2", type="experience", text="PRIVATE mixed",
             metadata={"session_id": "requested", "agent_identity": "beta"}),
    ]
    proc, requests, output = run_installed_sdk_case(profile, tmp_path,
                                                    config_overrides=config,
                                                    response_results=mixed)
    ...
    assert (method, path) == ("POST", "/v1/default/banks/hermes-alpha/memories/recall")
    assert body["tags"] == ["session:requested"] and body["tags_match"] == "all_strict"
    report = json.loads(output.read_text())["reconstruction"]
    assert report["error"] == "reconstruction-provenance-denied"
    assert report["facts"] == []
    assert "PRIVATE mixed" not in output.read_text()


@pytest.mark.parametrize("config_overrides,expected_error", [
    ({"mode": "local_external", "api_url": "http://synthetic.invalid",
      "bank_id_template": "", "bank_id": None}, "reconstruction-config-unavailable"),
    ({"mode": "local_external", "api_url": "http://synthetic.invalid",
      "bank_id": "hermes-alpha", "bank_id_template": "hermes-{profile}-{workspace}"},
     "reconstruction-profile-bank-not-isolated"),
])
def test_installed_sdk_missing_or_malformed_bank_config_never_queries(profile, tmp_path,
                                                                      config_overrides,
                                                                      expected_error):
    proc, requests, output = run_installed_sdk_case(profile, tmp_path,
                                                    config_overrides=config_overrides)

...
class TestTrackForgetQuick:
    ...
    def test_quick_wildcard_tolerates_file_disappearing_during_planning(
        self, _isolate_env, monkeypatch
    ):
        dg = _load_lib()
        root = _isolate_env / "audio_cache"
        root.mkdir()
        disappearing = root / "vanishing.ogg"
        ...
        def vanish_during_stat(path, *args, **kwargs):
        ...
        monkeypatch.setattr(Path, "stat", vanish_during_stat)

        summary = dg.quick()

        assert summary["deleted"] == 0
        assert summary["errors"] == []
        assert dg.load_tracked()[0]["path"] == f"{root.resolve()}/*"

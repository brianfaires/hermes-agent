...
async def test_compress_preserves_parent_when_child_creation_fails(tmp_path, monkeypatch):
    ...
    # Current compression architecture rolls the live id back immediately
    # when child creation fails; no phantom child may escape.
    assert agent.session_id == "sess-1"
...
async def test_compress_preserves_parent_when_transcript_write_fails(tmp_path, monkeypatch):
    ...
    runner, store, entry = _make_real_runner(tmp_path, monkeypatch, history)
    db = runner._session_db
    create_session = MagicMock(wraps=db.create_session)
    ...
    new_sid = create_session.call_args.kwargs["session_id"]
    assert new_sid != "sess-1"
    assert agent.session_id == "sess-1"
...
def test_compression_rollback_preserves_nonempty_child_by_default(
    tmp_path, monkeypatch
):
    """The generic rollback helper must not erase a child with durable rows."""
    runner, _store, _entry = _make_real_runner(
        tmp_path, monkeypatch, _make_history()
    )
    db = runner._session_db
    db.end_session("sess-1", "compression")
    db.create_session(
        session_id="child-1",
        source="test",
    ...
    db.replace_messages(
        "child-1", [{"role": "user", "content": "durable child work"}]
    )

    assert not db.discard_failed_compression_child("sess-1", "child-1")
    assert db.get_session("child-1") is not None
    assert db.get_session("sess-1")["end_reason"] == "compression"


@pytest.mark.asyncio
async def test_compress_rolls_back_when_gateway_followup_write_fails(
    tmp_path, monkeypatch
):
    """A failure after the core child write must still restore the live parent."""
    history = _make_history()
    compressed = [{"role": "user", "content": "summary of the conversation"}]
    runner, store, entry = _make_real_runner(tmp_path, monkeypatch, history)
    db = runner._session_db
    ...
    child_sid = create_session.call_args.kwargs["session_id"]
    assert child_sid != "sess-1"
    assert db.get_session(child_sid) is None
    assert agent.session_id == "sess-1"
    assert entry.session_id == "sess-1"
    assert db.get_session("sess-1")["ended_at"] is None
    assert db.resolve_resume_session_id("sess-1") == "sess-1"
    assert [m["content"] for m in store.load_transcript("sess-1")] == [
        "one",
        "two",
        "three",
        "four",

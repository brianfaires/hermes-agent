...
class TestLoadGatewayConfig:
    ...
    def test_discord_allowed_channels_seed_platform_extra(self, tmp_path, monkeypatch):
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        (hermes_home / "config.yaml").write_text(
        ...
        assert config.platforms[Platform.DISCORD].extra["allowed_channels"] == "111,222"

    def test_discord_ignored_channels_seed_platform_extra(self, tmp_path, monkeypatch):
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        (hermes_home / "config.yaml").write_text(
    ...
    def test_bridges_discord_auto_voice_config_from_top_level_discord(self, tmp_path, monkeypatch):
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        config_path = hermes_home / "config.yaml"
        config_path.write_text(

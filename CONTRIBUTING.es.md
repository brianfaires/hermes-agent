...
| **Node.js 22.22+** | Opcional — necesario para herramientas de navegador y las aplicaciones de escritorio/web (coincide con los engines de `package.json` raíz) |
...
1. **`description` ≤ 1024 caracteres, una oración, termina con punto.** Las descripciones largas saturan la UI de listado de habilidades, así que sé conciso en vez de tratar el límite del esquema como un objetivo. Indica la capacidad, no la implementación. Sin palabras de marketing ("potente", "completo", "fluido", "avanzado").
